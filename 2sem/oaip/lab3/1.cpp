    ...
    int main(int argc, char* argv[]) {
        ...
        MenuFunc menu[] = {
            ...
            [](rabotyaga*& arr, int& count) {
                ...
                for (int i = 0; i < n; i++) temp_nums[i] = nums[i];
                cout << "Выбранные работяги:" << endl;
                for (int i = 0; i < n; i++) {
                    if (temp_nums[i] >= 1 && temp_nums[i] <= count) {
                        cout << "Работяга " << temp_nums[i] << ": " 
                            << arr[temp_nums[i]-1].get_scnd_name() << " "
                            << arr[temp_nums[i]-1].get_name() << " "
                            << arr[temp_nums[i]-1].get_surname() << endl;
                    }
                }
                delete[] temp_nums;
            },
        ...
        while (true) {
            cout << "Меню" << endl
                ...
                << "8 - Множественный поиск" << endl << "0 - Выход " << endl;
            int c;
            cin >> c;
            if (c >= 1 && c < SIZE) {
